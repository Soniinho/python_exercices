...
word_list = ['PROGRAMACAO', 'DESENVOLVIMENTO', 'INTELIGENCIA', 'ARTIFICIAL', 'COMPUTADOR', 'ALGORITMO']
...
class HangmanGame:
    ...
    def create_widgets(self):
        ...
        self.reset_button = tk.Button(self.root, text="Próxima Rodada", command=self.check_letter)
        self.reset_button.pack()

        self.message_label = tk.Label(self.root, text='', font=('Helvetica', 12))
        self.message_label.pack()

        self.message_label1 = tk.Label(self.root, text='', font=('Helvetica', 12))
        self.message_label1.pack()
    ...
    def check_letter(self):
        sugerir_letra = self.prolog.query(f"once(sugerir_letra({self.wordAs}, Letra))")
        letra = next(sugerir_letra)["Letra"]
        sugerir_letra = set()

        while letra.upper() in self.guessed_letters:
            sugerir_letra = self.prolog.query(f"once(sugerir_letra({self.wordAs}, Letra))")
            letra = next(sugerir_letra)["Letra"]
            sugerir_letra = set()

        self.message_label1.config(text=f"A letra escolhida foi {letra}")

        if letra in self.guessed_letters: #letter
            self.message_label.config(text="Você já tentou essa letra!")
        elif letra in self.word:#letter
            self.guessed_letters.add(letra)#letter
            self.message_label.config(text="Letra correta!")
        else:
